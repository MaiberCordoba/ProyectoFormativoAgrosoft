...
const FormComponentF: React.FC<FormComponentProps> = ({
  fields,
  onSubmit,
  submitLabel = "Enviar",
  extraContent,
    ...
    <Form className="w-full max-w-xs flex flex-col gap-4" onSubmit={handleSubmit}>
      {fields.map((field) => {
        if (field.options) {
          ...
          const options = [
            { key: "AS", label: "Asignada", value: true },
            { key: "CO", label: "Completada", value: true },
            { key: "CA", label: "Cancelada", value: true },
          ];
