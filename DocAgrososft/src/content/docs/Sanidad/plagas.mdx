---
title: Plagas
description: Endpoint para gestionar plagas en AgroSoft.
---

import ApiBaseBanner from "../../../componets/ApiEndpoint.astro";

### Descripción

Este endpoint permite la gestión de plagas registradas en el sistema AgroSoft. Incluye operaciones de consulta, creación, actualización.

## Métodos

### 🔹 Método GET

<ApiBaseBanner
  method="GET"
  path="/plaga/"
  baseUrl="http://127.0.0.1:8000"
/>

Este método devuelve la lista de todas las plagas registradas.

###### ✅ **Respuesta exitosa (200)**
```json
{
  "id": 1,
  "tipoPlaga": {
    "id": 2,
    "nombre": "Pulgón Verde",
    "descripcion": "Insecto pequeño que afecta cultivos de trigo y cebada.",
    "img": "https://example.com/pulgon_verde.jpg"
  },
  "nombre": "Control Biológico",
  "descripcion": "Uso de mariquitas para reducir la población de pulgones en los cultivos.",
  "img": "https://example.com/control_biologico.jpg",
  "fk_Tipo": 2
}
```
###### ❌ Respuesta Error (500 - Internal Server Error)

### 🔹 Método POST

<ApiBaseBanner
  method="POST"
  path="/plaga/"
  baseUrl="http://127.0.0.1:8000"
/>

Este método permite registrar una nueva plaga.

### **Parámetros del Request**

| Parámetro       | Tipo     | Requerido | Descripción            | Ejemplo          |
|----------------|----------|-----------|------------------------|------------------|
| `nombre`      | string   | Sí        | Nombre de la plaga     | "nueva plaga"   |
| `descripcion` | string   | Sí        | Descripción de la plaga | "descripción"   |
| `img`         | string   | No        | URL de imagen opcional | "imagen.png"   |
| `fk_Tipo`     | integer  | Sí        | ID del tipo de plaga   | 2               |

---

### **Campos Requeridos**
```json
{
  "nombre": "nueva plaga",
  "descripcion": "descripción de la plaga",
  "img": "imagen.png",
  "fk_Tipo": 2
}
```

###### ✅ Respuesta Exitosa (201 - Created)
```json
{
  "id": 3,
  "nombre": "plaga creada",
  "descripcion": "descripción",
  "img": "imagen.png",
  "fk_Tipo": 2
}
```
###### ❌ Respuesta Error (500 - Internal Server Error)

### 🔹 Método PATCH

<ApiBaseBanner
  method="PATCH"
  path="/plaga/{id}/"
  baseUrl="http://127.0.0.1:8000"
/>

Este método permite actualizar una plaga existente.

### **Parámetros del Request**
| Parámetro       | Tipo     | Requerido | Descripción               | Ejemplo                  |
|----------------|----------|-----------|---------------------------|--------------------------|
| `id`          | integer  | Sí        | ID de la plaga            | 1                        |
| `nombre`      | string   | No        | Nombre actualizado        | "plaga actualizada"     |
| `descripcion` | string   | No        | Descripción actualizada   | "nueva descripción"     |
| `img`         | string   | No        | URL de imagen actualizada | "nueva-imagen.png"      |
| `fk_Tipo`     | integer  | No        | ID del tipo de plaga      | 2                        |

###### ✅ Respuesta Exitosa (200 - Ok)
```json
{
  "id": 1,
  "nombre": "plaga actualizada",
  "descripcion": "modificada",
  "img": "imagen_modificada.png",
  "fk_Tipo": 2
}
```
###### 🔍 Respuesta ID No encontrado (404 - Not Found)
###### ❌ Respuesta Error (500 - Internal Server Error)


